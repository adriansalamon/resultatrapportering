...
if os.environ.get("CANVAS_API_KEY") is None:
    print("Du måste sätta miljövariabeln CANVAS_API_KEY till din Canvas API-nyckel.")
    sys.exit(1)
else:
    g_access_token = os.environ.get("CANVAS_API_KEY")
...
class Course:
    def __init__(self, course):
        ...
        self.name_original = (
            course["original_name"] if "original_name" in course else None
        )
    ...
    def get_students(self):
        if self.__students is None:
            course_students = get_list(
                "/courses/" + str(self.id) + "/users?enrollment_type[]=student"
            )

            self.__students = [
                Student(course_student) for course_student in course_students
            ]
    ...
    def get_assignments(self):
        if self.__assignments is None:
            ...
            if "errors" in assignments:
                print(
                    "fel vid inläsning av uppgifter -- kanske fel API-nyckel eller fel kurs-ID?"
                )
            ...
            for assignment in assignments:
                if not assignment["published"]:
                    continue
                if assignment["grading_type"] not in [
                    "pass_fail",
                    "points",
                    "letter_grade",
                ]:
                ...
                if assignment["grading_standard_id"] is not None:
                    grading_scheme = self.get_grading_scheme(
                ...
                grades_affect_group = (
                    ...
                    and assignment["group_category_id"] is not None
                )
    ...
    def get_grading_scheme(self, id):
        ...
        if id not in g_grading_schemes:
            grading_standard = get_object(
                "/courses/" + str(self.id) + "/grading_standards/" + str(id)
            )
            g_grading_schemes[id] = (
                [grade["name"] for grade in grading_standard["grading_scheme"]]
                if "grading_scheme" in grading_standard
                else None
            )
...
class Assignment:
    def __init__(
        self, course, id, name, grading_type, grading_scheme, grades_affect_group
    ):
        self.course = course
        self.id = id
        self.name = name
...
class Student:
    ...
    def get_results(self, course, force_upgrade=False):
        if course in self.__results and not force_upgrade:
    ...
    def get_result(self, assignment, force_upgrade=False):
        current_grades = self.get_results(
            assignment.course, force_upgrade=force_upgrade
        )
        return (
            current_grades[assignment]
            if assignment in current_grades
            else {"grade": "-", "date": None}
        )
...
def get_list(url):
    ...
    while url is not None:
        if not url.startswith(g_base):
            url = g_base + url

        if "?" in url:
            url = url.replace("?", "?per_page=100&")
            url += "&access_token=" + g_access_token
        else:
            url += "?per_page=100&access_token=" + g_access_token

        response_this = requests.get(url=url)
...
def get_object(url):
    return requests.get(
        url=g_base + url, headers={"Authorization": "Bearer " + g_access_token}
    ).json()
...
def get_courses(search_term=None):
    ...
    all_courses = [
        Course(course)
        for course in get_list("/courses")
        if len([x for x in course["enrollments"] if x["type"] != "student"]) > 0
    ]

    return (
        all_courses
        if search_term is None
        else [course for course in all_courses if search_term in course]
    )
